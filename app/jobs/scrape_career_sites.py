...
def main() -> int:
    ...
    for competitor in competitors:
        ...
        print(
            f"[career_page] {competitor.slug:20s} seen={result.items_seen:>3} added={result.items_added:>3}"
            + (f" WARN: {warnings}" if warnings else "")
            + (f" ERROR: {result.error}" if result.error else "")
        )

    print(f"\n[career_page] Total: {len(competitors)} konkurrenter, {total_added} nye opslag.")
    if failures:
        return 1
    return 0
